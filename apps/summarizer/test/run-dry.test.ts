...
it.effect("dry-run distinguishes an unfinished floor from a settled recent floor", () =>
  Effect.gen(function* () {
    ...
    const recent = yield* setup({}, config.replace('id: "10"', 'id: "11"'));
    recent.discord.addChannel("11");
    const settings = yield* decodeConfig(config, "/home/test");
    const empty = yield* openChannelRecord("11", settings.since, settings.horizon);
    const recentFloor = (BigInt(snowflakeAt(at - day)) - 1n).toString();
    yield* updateRecord(empty, { ...empty.record, floor: recentFloor });
...
it.effect("dry-run labels exact Since and Horizon floors and an unfinished scan", () =>
  Effect.gen(function* () {
    ...
    for (const [floorAt, label] of [
      ...
      yield* updateRecord(journal, { ...journal.record, floor });
      const report = captureLogs();
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      journal = yield* journalPage(journal, [terminal.id]);
      journal = yield* journalStatus(journal, {
        id: terminal.id,
        state: "terminal",
      });
      const report = captureLogs();
      ...
      let prior = yield* openChannelRecord("10", priorConfig.since, priorConfig.horizon);
      prior = yield* journalPage(prior, [before.id, after.id]);
      yield* journalStatus(prior, { id: after.id, state: "terminal" });
      ...
      yield* journalPage(prior, [due.id]);
...
it.effect("dry-run counts a journaled old leftover alongside new Pending and Given-up work", () =>
  Effect.gen(function* () {
    ...
    yield* journalPage(journal, [oldPending.id, oldGiven.id]);
...
it.effect("dry-run skips terminal and already-seen references and tolerates deleted sources", () =>
  Effect.gen(function* () {
    ...
    journal = yield* journalPage(journal, [deleted.id, terminal.id]);
    yield* journalStatus(journal, { id: terminal.id, state: "terminal" });
    discord.messages.set(
      "10",
...
for (const [minutes, partial] of [
  ...
  it.effect(`dry-run ${partial ? "labels" : "completes"} a ${minutes}-minute journal lookup`, () =>
    Effect.gen(function* () {
      ...
      yield* journalPage(journal, [newer.id]);
      discord.faults.push({
        method: "GET",
