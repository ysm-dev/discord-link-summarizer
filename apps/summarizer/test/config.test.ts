...
it.effect("decodes defaults, channel overrides, timezone offsets and attempt cap", () =>
  Effect.gen(function* () {
    ...
    expect(defaults.progressDatabase).toBe(
      "/Users/me/.local/state/discord-link-summarizer/progress.sqlite",
    );
  ...
  () =>
    Effect.gen(function* () {
      for (const text of [
        ...
        minimal + "state_channel_id: '999'\n",
        minimal.replace("id: '123'", "id: 'x123'"),
        minimal.replace("id: '123'", "id: '123x'"),
      ...
      expect(
        (yield* Effect.flip(decodeConfig(minimal.replace("id: '123'", "id: 'x123'"), "/Users/me")))
          .message,
